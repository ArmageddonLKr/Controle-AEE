...
import {
  getCriancaById,
  getSessoesByCriancaId,
  getEvolucoesByCriancaId,
  getTaxaPresenca,
} from "@/lib/mock-data";
...
const CORES_AVATAR = [
  { bg: "#E0F4F8", text: "#1E3A5F" },
  { bg: "#D1F0E8", text: "#1A5F3F" },
  { bg: "#FAE5D3", text: "#7D3B0A" },
  { bg: "#E8E0F8", text: "#3B1A7D" },
  { bg: "#FCE4EC", text: "#7D1A2F" },
  { bg: "#E0EBF8", text: "#1A3D7D" },
  { bg: "#FFF3CD", text: "#7D5A00" },
  { bg: "#D4EDDA", text: "#1A5F30" },
];
...
const TIPO_SESSAO_CONFIG: Record<string, { label: string; bg: string; color: string }> = {
...
export function AlunoPerfilClient({ id }: { id: string }) {
  const crianca = getCriancaById(id);
  const sessoes = getSessoesByCriancaId(id);
  const evolucoes = getEvolucoesByCriancaId(id);
  const taxaPresenca = getTaxaPresenca(id);
  ...
  return (
    <div style={{ display: "flex", gap: "1.5rem", alignItems: "flex-start", minHeight: "100%" }}>
      {/* COLUNA LATERAL */}
      <aside style={{ width: "240px", minWidth: "240px", display: "flex", flexDirection: "column", gap: "0", background: "var(--bg-card)", borderRadius: "0.75rem", boxShadow: "0 1px 3px rgba(0,0,0,0.06), 0 4px 16px rgba(0,0,0,0.04)", border: "1px solid var(--border)", overflow: "hidden" }}>
        ...
        <div style={{ padding: "1.25rem 1rem 1rem", display: "flex", flexDirection: "column", alignItems: "center", gap: "0.625rem", textAlign: "center", borderBottom: "1px solid var(--border)" }}>
          <div style={{ width: "64px", height: "64px", borderRadius: "50%", background: cor.bg, color: cor.text, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.375rem", fontWeight: 700 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
            ...
            <span style={{ display: "inline-block", padding: "0.15rem 0.6rem", borderRadius: "9999px", fontSize: "0.7rem", fontWeight: 700, background: statusCfg.bg, color: statusCfg.color, marginTop: "0.125rem" }}>{statusCfg.label}</span>
          </div>
        </div>
        ...
        <div style={{ padding: "0.875rem 1rem", display: "flex", flexDirection: "column", gap: "0.625rem", borderBottom: "1px solid var(--border)" }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: "0.5rem" }}>
            <Calendar size={13} style={{ color: "var(--text-muted)", marginTop: "2px", flexShrink: 0 }} />
            <div>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: "0.5rem" }}>
            <School size={13} style={{ color: "var(--text-muted)", marginTop: "2px", flexShrink: 0 }} />
            <div>
        ...
        <div style={{ padding: "0.875rem 1rem", borderBottom: "1px solid var(--border)" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.3rem" }}>
            {crianca.diagnosticos.map((d) => (
              <span key={d} style={{ padding: "0.2rem 0.5rem", borderRadius: "9999px", fontSize: "0.68rem", fontWeight: 600, background: "var(--accent-light)", color: "var(--accent-primary)" }}>{d}</span>
            ))}
          </div>
        </div>
            ...
            {crianca.responsaveis.map((r, i) => (
              <div key={i}>
                ...
                <div style={{ paddingLeft: "1.25rem", display: "flex", flexDirection: "column", gap: "0.15rem" }}>
                  <span style={{ fontSize: "0.68rem", color: "var(--text-muted)" }}>{r.parentesco}{r.responsavelLegal && <span style={{ marginLeft: "0.35rem", padding: "0.05rem 0.35rem", borderRadius: "9999px", background: "var(--accent-light)", color: "var(--accent-primary)", fontSize: "0.6rem", fontWeight: 700 }}>legal</span>}</span>
        ...
        {/* Início + observações + medicamentos */}
        ...
        {crianca.observacoesImportantes && (
          <div style={{ margin: "0.75rem", padding: "0.625rem 0.75rem", borderRadius: "0.5rem", background: "#FFF8E1", border: "1px solid #F0A500" }}>
            <p style={{ fontSize: "0.65rem", fontWeight: 700, color: "#7D5A00", marginBottom: "0.3rem" }}>⚠️ Observações importantes</p>
        ...
        <div style={{ padding: "0.875rem 1rem", borderTop: "1px solid var(--border)", display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "0.25rem", textAlign: "center", marginTop: "auto" }}>
          <div><div style={{ fontSize: "1.25rem", fontWeight: 800, color: "var(--accent-primary)" }}>{sessoes.length}</div><div style={{ fontSize: "0.6rem", color: "var(--text-muted)", lineHeight: "1.3" }}>Sessões</div></div>
          <div><div style={{ fontSize: "1.25rem", fontWeight: 800, color: "var(--success)" }}>{taxaPresenca}%</div><div style={{ fontSize: "0.6rem", color: "var(--text-muted)", lineHeight: "1.3" }}>Presença</div></div>
          <div><div style={{ fontSize: "1.25rem", fontWeight: 800, color: "var(--accent-secondary)" }}>{evolucoes.length}</div><div style={{ fontSize: "0.6rem", color: "var(--text-muted)", lineHeight: "1.3" }}>Evoluções</div></div>
        </div>
      </aside>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: "1rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.25rem", borderBottom: "2px solid var(--border)" }}>
          {([
            { key: "sessoes", label: "📓 Sessões", count: sessoes.length },
            { key: "evolucao", label: "📈 Evolução", count: evolucoes.length },
            { key: "informacoes", label: "📋 Informações", count: null },
            { key: "exportar", label: "📤 Exportar", count: null },
          ] as const).map((aba) => {
            const ativa = abaAtiva === aba.key;
            return (
              <button key={aba.key} onClick={() => setAbaAtiva(aba.key)} style={{ padding: "0.625rem 1rem", borderRadius: "0.5rem 0.5rem 0 0", border: "none", borderBottom: ativa ? "2px solid var(--accent-primary)" : "2px solid transparent", background: ativa ? "var(--accent-light)" : "transparent", color: ativa ? "var(--accent-primary)" : "var(--text-muted)", fontWeight: ativa ? 700 : 500, fontSize: "0.8rem", cursor: "pointer", transition: "all 200ms ease", display: "flex", alignItems: "center", gap: "0.375rem", marginBottom: "-2px" }}>
                {aba.label}
                {aba.count !== null && <span style={{ padding: "0.1rem 0.4rem", borderRadius: "9999px", fontSize: "0.65rem", fontWeight: 700, background: ativa ? "var(--accent-primary)" : "var(--border)", color: ativa ? "#fff" : "var(--text-muted)" }}>{aba.count}</span>}
              </button>
            );
          })}
        </div>
        ...
        {abaAtiva === "sessoes" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <button disabled title="Disponível em breve" style={{ padding: "0.4rem 0.875rem", borderRadius: "0.5rem", border: "1px solid var(--border)", background: "var(--bg-card)", color: "var(--text-muted)", fontSize: "0.8rem", fontWeight: 600, cursor: "not-allowed", opacity: 0.6 }}>+ Nova Sessão</button>
            </div>
            {sessoes.length === 0 ? (
              <div style={{ padding: "3rem 1.5rem", textAlign: "center", border: "2px dashed var(--border)", borderRadius: "0.75rem", color: "var(--text-muted)", fontSize: "0.875rem" }}>Nenhuma sessão registrada ainda.</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.625rem" }}>
                {sessoes.map((s) => {
                  const tipoCfg = TIPO_SESSAO_CONFIG[s.tipo] ?? TIPO_SESSAO_CONFIG.individual;
                  return (
                    <div key={s.id} className="card-aee" style={{ padding: "0.875rem 1rem", display: "flex", flexDirection: "column", gap: "0.5rem", borderLeft: s.presente ? "3px solid var(--success)" : "3px solid var(--danger)" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.625rem", flexWrap: "wrap" }}>
                        ...
                        <span style={{ padding: "0.15rem 0.5rem", borderRadius: "9999px", fontSize: "0.68rem", fontWeight: 700, background: tipoCfg.bg, color: tipoCfg.color }}>{tipoCfg.label}</span>
        ...
        {abaAtiva === "evolucao" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
            ...
            {evolucoes.length === 0 ? (
              <div style={{ padding: "3rem 1.5rem", textAlign: "center", border: "2px dashed var(--border)", borderRadius: "0.75rem", color: "var(--text-muted)" }}>Nenhum registro de evolução ainda.</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
                {evolucoes.map((e) => (
                  <div key={e.id} className="card-aee" style={{ padding: "1rem 1.125rem", display: "flex", flexDirection: "column", gap: "0.625rem" }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.3rem" }}>
                      {e.areas.map((area) => <span key={area} style={{ padding: "0.2rem 0.55rem", borderRadius: "9999px", fontSize: "0.68rem", fontWeight: 600, background: "var(--accent-light)", color: "var(--accent-primary)" }}>{area}</span>)}
                    </div>
                ...
                {crianca.responsaveis.map((r, i) => (
                  <div key={i} style={{ padding: "0.75rem", borderRadius: "0.5rem", background: "var(--bg-primary)", border: "1px solid var(--border)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
                      ...
                      <span style={{ padding: "0.1rem 0.4rem", borderRadius: "9999px", fontSize: "0.65rem", fontWeight: 600, background: "var(--accent-light)", color: "var(--accent-primary)" }}>{r.parentesco}</span>
                      {r.responsavelLegal && <span style={{ padding: "0.1rem 0.4rem", borderRadius: "9999px", fontSize: "0.6rem", fontWeight: 700, background: "var(--success)", color: "#fff" }}>responsável legal</span>}
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
                <div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem" }}>
                    {crianca.diagnosticos.map((d) => <span key={d} style={{ padding: "0.25rem 0.625rem", borderRadius: "9999px", fontSize: "0.75rem", fontWeight: 600, background: "var(--accent-light)", color: "var(--accent-primary)" }}>{d}</span>)}
                  </div>
                </div>
                ...
                {crianca.alergias && crianca.alergias.length > 0 && (
                  <div>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem" }}>
                      {crianca.alergias.map((a) => <span key={a} style={{ padding: "0.25rem 0.625rem", borderRadius: "9999px", fontSize: "0.75rem", fontWeight: 600, background: "#FCE4EC", color: "#C62828" }}>{a}</span>)}
                    </div>
                  </div>
                )}
              </div>
        ...
        {abaAtiva === "exportar" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            <p style={{ fontSize: "0.875rem", color: "var(--text-muted)", margin: 0 }}>Gere e baixe a ficha completa de {crianca.nome} no formato desejado.</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
              <button onClick={handleExportPdf} disabled={loadingPdf} style={{ padding: "1.5rem", borderRadius: "0.75rem", border: "2px solid var(--border)", background: loadingPdf ? "var(--bg-primary)" : "var(--bg-card)", cursor: loadingPdf ? "wait" : "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: "0.75rem", transition: "all 200ms ease", opacity: loadingPdf ? 0.7 : 1 }}>
                <div style={{ width: "52px", height: "52px", borderRadius: "0.75rem", background: "#FFEBEE", display: "flex", alignItems: "center", justifyContent: "center" }}><Download size={24} style={{ color: "#C62828" }} /></div>
                <div style={{ textAlign: "center" }}>
                  <p style={{ margin: 0, fontSize: "0.9rem", fontWeight: 700, color: "var(--text-primary)" }}>{loadingPdf ? "Gerando PDF..." : "Exportar PDF"}</p>
                  <p style={{ margin: "0.25rem 0 0", fontSize: "0.75rem", color: "var(--text-muted)" }}>Ficha completa + histórico de sessões</p>
                </div>
              </button>
              <button onClick={handleExportDocx} disabled={loadingDocx} style={{ padding: "1.5rem", borderRadius: "0.75rem", border: "2px solid var(--border)", background: loadingDocx ? "var(--bg-primary)" : "var(--bg-card)", cursor: loadingDocx ? "wait" : "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: "0.75rem", transition: "all 200ms ease", opacity: loadingDocx ? 0.7 : 1 }}>
                <div style={{ width: "52px", height: "52px", borderRadius: "0.75rem", background: "#E3F2FD", display: "flex", alignItems: "center", justifyContent: "center" }}><FileText size={24} style={{ color: "#1565C0" }} /></div>
                <div style={{ textAlign: "center" }}>
